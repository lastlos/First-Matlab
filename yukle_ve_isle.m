function [X_egitim, X_test, y_egitim, y_test, ozellik_adlari, sinif_adlari] = yukle_ve_isle(test_orani, tohum)


ozellik_adlari = {'Canak Uzunlugu (cm)', 'Canak Genisligi (cm)', ...
                  'Tac Uzunlugu (cm)',   'Tac Genisligi (cm)'};
...
meas = [setosa; versicolor; virginica];
y_ham = [ones(50,1); 2*ones(50,1); 3*ones(50,1)];
...
test_indeks   = [];
for s = 1:3
    sinif_idx    = find(y_ham == s);
    n_sinif      = length(sinif_idx);
    karisik_idx  = sinif_idx(randperm(n_sinif));
    n_egitim     = round(n_sinif * (1 - test_orani));
    egitim_indeks = [egitim_indeks; karisik_idx(1:n_egitim)];
    test_indeks   = [test_indeks;   karisik_idx(n_egitim+1:end)];
end

X_egitim_ham = meas(egitim_indeks, :);
X_test_ham   = meas(test_indeks,   :);
y_egitim     = y_ham(egitim_indeks);
y_test       = y_ham(test_indeks);

min_eg  = min(X_egitim_ham);
max_eg  = max(X_egitim_ham);
aralik  = max_eg - min_eg;
aralik(aralik == 0) = 1;

X_egitim = (X_egitim_ham - min_eg) ./ aralik;
X_test   = (X_test_ham   - min_eg) ./ aralik;
